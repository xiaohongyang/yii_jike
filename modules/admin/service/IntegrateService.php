...
class IntegrateService extends BaseService
{
    ...
    public $model;
    public function __construct(User_int_account_flow $model=null)
    {
        $this->model = $model;
    }
    ...
    public function getList(){
        {
            ...
            $query = User_int_account_flow::find();
            $query->from('jike_user_int_account_flow flow');
//User_int_account_flow_cashuser::

            if(is_null($status) || $status!=1){
                $query->where(['flow.is_paid' => User_int_account_flow::IS_PAYED_FALSE]);
            } else {
                $query->where(['flow.is_paid' => User_int_account_flow::IS_PAYED_TRUE]);
            }
            $query->leftJoin('jike_user_int_account_flow_cashuser cashuser', 'flow.id = cashuser.flow_id');
