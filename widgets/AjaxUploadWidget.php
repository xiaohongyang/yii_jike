...
class AjaxUploadWidget extends Widget{
    ...
    public $loadSrc;    //= !is_null($loadSrc) ? : '/images/default-headpic.png';
    public $ajaxParamId;    // = is_null($ajaxParamId)?"ajax_param_id" : $ajaxParamId;
    public $dataId; // = is_null($dataId) ? "ajax_data_id" : $dataId;
    public $fileDesc;   // = is_null($fileDesc)?'':$fileDesc;
    public $fileType;   // = is_null($fileType) ? '' : $fileType;
    public $uploadIdValue;  // = !is_null($uploadIdValue)?:'0';
    public $isShowDelBtn = false;
    ...
    public function run()
    {
        ...
        $loadSrc= !is_null($this->loadSrc) ? $this->loadSrc : '/images/default-headpic.png';
        $ajaxParamId = !is_null($this->ajaxParamId)? $this->ajaxParamId : "ajax_param_id";
        $dataId = !is_null($this->dataId) ? $this->dataId : "ajax_data_id";
        $fileDesc = !is_null($this->fileDesc)? $this->fileDesc : '';
        $fileType = !is_null($this->fileType) ? $this->fileType : '';
        $uploadIdValue = !is_null($this->uploadIdValue)? $this->uploadIdValue :'0';

        return $this->render('AjaxUploadWidget',[
            'loadSrc' => $loadSrc,
            'ajaxParamId' => $ajaxParamId,
            'dataId' => $dataId,
            'fileDesc' => $fileDesc,
            'fileType' => $fileType,
            'uploadIdValue' => $uploadIdValue,
            'isShowDelBtn' => $this->isShowDelBtn
        ]);
    }
