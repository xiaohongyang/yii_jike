...
//			echo "Could not open " . $refresh_file . "!\n";
	...
	/**
	 * @param bool $upload_process
	 * @param array $params
	 * @param array $uploadInfo
	 * @return array|mixed $result['status','videoid']
	 */
	public function upload($upload_process = true, $params = array(),$uploadInfo = array()) {

		$result = ['status'=>0, 'videoid' => 0];
		...
		if( property_exists($uploadResult, 'video_id') && $uploadResult->video_id){
			$result['status'] = 1;
			$result['videoid'] = $uploadResult->video_id;
		} else {
...
//			if (isset($commitResult->video_id)) echo "videoid: ". $commitResult->video_id."\n";
				$result['status'] = 1;
				$result['videoid'] = $commitResult->video_id;
			}
		}
		return $result;
	}
