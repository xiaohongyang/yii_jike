...
//	require_once SCRIPT_ROOT.'include/Client.php';
	...
	define('SCRIPT_ROOT',  dirname(__FILE__).'/');
//echo SCRIPT_ROOT .''. 'Client.php';exit;
 ...
 * 序列号,请通过亿美销售人员获取
 */
$serialNumber = '3SDK-EMY-0130-PJYRK';
...
$client_msg = new Client($gwUrl,$serialNumber,$password,$sessionKey,$proxyhost,$proxyport,$proxyusername,$proxypassword,$connectTimeOut,$readTimeOut);
 
/**
 * 发送向服务端的编码，如果本页面的编码为utf-8，请使用utf-8
 */
$client_msg->setOutgoingEncoding("utf-8");
...
function login()
{
	 ...
	 * 我们建议 sesson key不用常变
	 */
	//$sessionKey = $client_msg->generateKey();
	//$statusCode = $client_msg->login($sessionKey);
	
	$statusCode = $client_msg->login();
	
	echo "处理状态码:".$statusCode."<br/>";
	if ($statusCode!=null && $statusCode=="0")
	{
		//登录成功，并且做保存 $sessionKey 的操作，用于以后相关操作的使用
		echo "登录成功, session key:".$client_msg->getSessionKey()."<br/>";
	}else{
		//登录失败处理
		echo "登录失败";
	}
...
function cancelMOForward()
{
	...
	$statusCode = $client_msg->cancelMOForward();
	echo "处理状态码:".$statusCode;
}
...
function chargeUp()
{
	...
	$cardPass = '123456';
	$statusCode = $client_msg->chargeUp($cardId,$cardPass);
	echo "处理状态码:".$statusCode;
}
...
function registDetailInfo()
{
	 ...
	 * 
	 */
	$statusCode = $client_msg->registDetailInfo($eName,$linkMan,$phoneNum,$mobile,$email,$fax,$address,$postcode);
	echo "处理状态码:".$statusCode;
...
function updatePassword()
{
	 ...
	 */
	$statusCode = $client_msg->updatePassword('654321');
	echo "处理状态码:".$statusCode;
}
...
function setMOForward()
{
	 ...
	 */	
	$statusCode = $client_msg->setMOForward('159xxxxxxxx');
	echo "处理状态码:".$statusCode;
}
...
function getMO()
{
	...
	foreach($moResult as $mo)
	{
		...
		// 实例代码为直接输出
	 	 ...
	 	 */
	 	echo "短信内容:".iconv("UTF-8","utf-8",$mo->getSmsContent());
 ...
 * 短信发送 用例
 */
function sendSMS($arr_tel,$content)
{
	global $client_msg;
	/**
	 * 下面的代码将发送内容为 test 给 159xxxxxxxx 和 159xxxxxxxx
	 * $client_msg->sendSMS还有更多可用参数，请参考 Client.php
	 */
	$statusCode = $client_msg->sendSMS($arr_tel,$content);
//	var_dump($client_msg->getBalance());
	return $statusCode;
}
...
function setMOForwardEx()
{
	 ...
	 * 以数组形式填写手机号码
	 */	
	$statusCode = $client_msg->setMOForwardEx(
